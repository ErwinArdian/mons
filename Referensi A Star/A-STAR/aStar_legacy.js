...
class aStar{
	...
	search(){
		...
		path.push(this.endPos);	//push end pos to array
...
var pixelColor = "#00ff00"; //by default green for start position
var startPos = [];
var endPos = [];
var wallPos = [];
//define array
...
canvas.addEventListener('click', function(e){
		...
		context.fillRect(Math.floor(e.offsetX/50)*50, Math.floor(e.offsetY/50)*50, 50, 50); //edit klik rectangle
		//context.fillRect(Math.floor(Math.random()*x)*50, Math.floor(Math.random()*y)*50, 50, 50); //with random func
		...
		function drawGrid(){
			...
			for (var row = 0; row < 10; row++) //baris
			{
			   for (var column = 0; column < 10; column++) //kolom
				{
				    var x = column * 50;
				    var y = row * 50;
